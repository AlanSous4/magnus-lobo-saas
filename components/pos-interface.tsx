...
import { ShoppingCart, CreditCard, Wallet, DollarSign, UtensilsCrossed, Coffee, X, Check } from "lucide-react"
...
export function POSInterface({ products, userId }: POSInterfaceProps) {
  ...
  const getTotalAmount = () => {
    return cart.reduce((total, item) => total + item.value * item.cartQuantity, 0)
  }
              ...
              >
                ...
                <CardContent className="p-4 pt-0">
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-bold text-orange-600">R$ {product.value.toFixed(2)}</span>
                ...
                <Card key={item.id}>
                  <CardContent className="p-3">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <h3 className="font-medium text-sm flex-1 leading-tight">{item.name}</h3>
                      <Button
                        variant="ghost"
                        size="icon"
        ...
        <div className="border-t bg-background p-4 space-y-4">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              ...
              <span>{cart.reduce((sum, item) => sum + item.cartQuantity, 0)}</span>
            </div>
            <Separator />
      ...
      <Dialog open={showSuccess} onOpenChange={setShowSuccess}>
        <DialogContent className="sm:max-w-sm">
          <div className="flex flex-col items-center justify-center p-6 text-center">
            ...
            <h3 className="text-xl font-semibold mb-2">Venda Realizada!</h3>

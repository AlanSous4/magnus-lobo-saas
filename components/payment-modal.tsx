...
export function PaymentModal({
  ...
  return (
    ...
    >
      ...
      <span 
        onKeyDownCapture={(e) => e.key === 'Escape' && !processando && onClose()} 
        tabIndex={-1} 
      />
      ...
      >
        <AnimatePresence mode="wait" initial={false}>
          {!sucesso ? (
            ...
            >
              <h2 className="text-2xl font-black mb-6 text-stone-800 italic uppercase tracking-tighter">
                Pagamento
              </h2>
  
              <div className="grid grid-cols-2 gap-2 mb-6">
                <div
                  className={`p-3 rounded-xl text-center bg-stone-100 transition-all duration-300 ${
                    !(pagamentos.length > 0 && saldoRestante > 0.01) ? "col-span-2" : "col-span-1"
                  }`}
                >
              ...
              <div className="flex gap-2 mb-6">
                ...
                <button
                  ...
                  className="bg-stone-800 text-white px-6 rounded-xl font-black text-[10px] uppercase hover:bg-black active:scale-95 cursor-pointer transition-transform"
                >
                  Add
                </button>
              </div>
                  ...
                  {pagamentos.map((p, idx) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3">
                        <span className="font-black text-stone-800 text-sm font-mono">
                          R$ {p.valor.toFixed(2)}
                        </span>
                        <button
                          onClick={() => removerPagamento(idx)}
                          className="text-red-500 font-bold hover:bg-red-50 w-6 h-6 rounded-full flex items-center justify-center cursor-pointer transition-colors"
                        >
                          ✕
                        </button>
                      </div>
              ...
              <div className="flex flex-col gap-2">
                ...
                <button
                  ...
                  className="text-stone-400 font-black py-2 uppercase text-[10px] tracking-widest hover:text-stone-600 cursor-pointer disabled:opacity-0 transition-all"
                >
                  Voltar
                </button>
              </div>
            ...
            >
              ...
              {onPrint && (
                <button
                  onClick={() => onPrint?.(pagamentos, itensParaRecibo)}
                ...
                >
                  ...
                  <span className="text-[8px] opacity-50 font-normal italic">USB / Bluetooth</span>
                </button>
              )}
  
              {mesaInfo && (
                <p className="text-stone-400 font-bold text-sm mt-4 uppercase tracking-tighter">
                  {mesaInfo} Liberada
                </p>
              )}
  
              <div className="w-full h-1.5 bg-stone-100 mt-8 rounded-full overflow-hidden">
                <motion.div
                  initial={{ width: 0 }}
                  animate={{ width: "100%" }}
